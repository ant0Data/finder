...
selected = option_menu(
    menu_title=None,
    options=["Finder", "Espace personnel", "Préparez vos entretiens", "Chatbot", "A propos de Finder", "Contact"],
    icons=["bullseye", "person-circle", "chat-right-dots", "chat-right-dots", "bullseye", "envelope"],
    menu_icon="cast",
    default_index=0,
...
# ===============================
if selected == "Chatbot":
    st.session_state.messages = []  # Réinitialise le chat pour l'onglet Chatbot
...
elif selected == "Préparez vos entretiens":
    # st.title("Préparez votre entretien d'embauche")

    # Affichage immédiat du menu avec uniquement "ChatCoach" visible
    sub_selected = option_menu(
        menu_title="",
        options=["Bienvenue dans ChatCoach !"],  # Afficher uniquement "ChatCoach"
        menu_icon="chat-right-dots",
        default_index=0,  # # Définit l'index par défaut à 0 pour ChatCoachle menu
        orientation="horizontal",
        styles={
            "container": {"background-color": "#ecf0f1"},
            "icon": {"color": "black", "font-size": "20px"},
            "nav-link": {"font-size": "18px", "text-align": "center", "color": "black"},
            "nav-link-selected": {"background-color": "#1abc9c"},
        }
    )

    # Afficher uniquement ChatCoach si sélectionné
    if sub_selected == "Bienvenue dans ChatCoach !":
        # st.subheader("Bienvenue dans ChatCoach !")
        st.write("Répondez aux questions pour vous entraîner aux entretiens.")
        ...
        interview_questions = [
            ...
            "Parlez-moi d'une situation où vous avez réussi à dénouer un problème complexe, où vous êtes parvenu à mener un projet jusqu’au bout",
            "Racontez-moi un projet professionnel dont vous êtes particulièrement fier.",
            "Quels résultats avez-vous obtenus dans vos précédentes expériences ?",
        ...
        if len(st.session_state.messages) == 0 or st.session_state.messages[-1]["role"] == "assistant":
            ...
            question = interview_questions[st.session_state.current_question_index]
        ...
        if prompt := st.chat_input("Votre réponse..."):
            ...
            def evaluate_response(question, response):
                prompt_text = f"""
Tu es un coach en entretien d'embauche. Ton rôle est d'évaluer la réponse du candidat à la question suivante et de lui donner des conseils pour s'améliorer.
TU dois vouvoyer le candidat. Appuie toi sur le contenu de son cv pour personaliser les questions.

Question : tu peux utiliser des questions disponibles dans la liste(EN RANDOM): {question}  MAIS TU PEUX AUSSI poser des questions de ton choix basée sur les infomations que tu as obtenues de l'utilisateur.
Réponse du candidat : {response}

Fais les choses suivantes :
1. Analyse la réponse du candidat.
2. Donne un feedback constructif sur la réponse.
3. Fournis une réponse modèle que le candidat pourrait utiliser pour améliorer sa réponse.
4. Termine par un conseil pour mieux répondre à cette question à l'avenir.
...
Conseil : [Conseil]
            ...
            question = interview_questions[st.session_state.current_question_index]
            feedback = evaluate_response(question, prompt)
            ...
            if st.session_state.current_question_index >= len(interview_questions):
                st.session_state.messages.append({"role": "assistant", "content": "🎉 Félicitations ! Vous avez terminé toutes les questions. Vous êtes maintenant prêt pour votre entretien !"})
                with st.chat_message("assistant"):
                    st.markdown("🎉 Félicitations ! Vous avez terminé toutes les questions. Vous êtes maintenant prêt pour votre entretien !")
            else:
                next_question = interview_questions[st.session_state.current_question_index]
                st.session_state.messages.append({"role": "assistant", "content": f"**Question suivante :** {next_question}\n\n*Conseil : Restez concis et pertinent.*"})
                with st.chat_message("assistant"):
                    st.markdown(f"**Question suivante :** {next_question}\n\n*Conseil : Restez concis et pertinent.*")

    # Ajouter les autres options plus tard Coaching Video, Interactive Coaching , mais elles ne sont pas encore visibles

    # elif sub_selected == "Coaching Video":
    #     st.title("Simulation d'entretien d'embauche")
    #     video_path = r"C:\Users\riads\Downloads\Projet3\entretien_embauche.mp4"
    #     if not os.path.exists(video_path):
    #         st.error(f"Le fichier vidéo '{video_path}' n'existe pas.")
    #     else:
    #         st.video(video_path)
    #         st.write("La vidéo est chargée avec succès.")
    # elif sub_selected == "Interactive Coaching":
    #     st.title("Interactive Coaching")
    #     st.write("Get real-time feedback from our AI coach.")

elif selected == "Finder":
...
>       3. **Apport pour l’entreprise** : mise en avant des bénéfices pour l’employeur, selon les exigences décrites dans `{job_offer_details}`.  
...
"""
    ...
    # st.title("Contact")
    # st.write("Besoin d'aide ou d'informations ? Envoyez-nous un message !")
    st.header(":mailbox: Besoin d'aide ou d'informations ? Envoyez-nous un message !")
    contact_form = """
    <form action="https://formsubmit.co/finderprojet3@gmail.com" method="POST">
        <input type="hidden" name="_captcha" value="false">
        <input type="hidden" name="_autoresponse" value="Thank you for reaching out to us! We have received your message and will get back to you as soon as possible.">
        <input type="hidden" name="_template" value="table">
        <!-- Zone de texte avec une taille agrandie (rows et cols) -->
        <textarea name="message" placeholder="Redigez votre message" required rows="6" cols="50"></textarea>
